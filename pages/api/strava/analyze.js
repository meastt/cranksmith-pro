...
export default async function handler(req, res) {
    ...
    try {
      const { accessToken, crankset, cassette } = req.body;
      ...
      // Check if this is a real access token or mock data
      if (accessToken.startsWith('mock_access_token_') || accessToken === 'demo_token') {
        // Generate demo activities for testing
        activities = generateDemoActivities();
      } else {
        // Fetch real activities from Strava
        console.log('Fetching real Strava data with token:', accessToken.substring(0, 10) + '...');
  ...
  function analyzeGearUsage(rides, crankset, cassette) {
    // Simulate gear usage analysis
    // In real implementation, you'd need power meter data or detailed activity streams
